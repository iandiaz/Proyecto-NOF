...
<html lang="en">
  ...
  if(request.getParameter("Exito")!=null && !request.getParameter("Exito").equals("")){
  		...
  		if(request.getParameter("Exito").equals("NOOK1")){
			out.println("<script>alert('ERROR: NO EXISTEN FOLIOS DISPONIBLES PARA ESTE TIPO DE DOCUMENTO')</script>");
			//out.println("<script>window.location = '/001/'</script>");
		} 
  ...
  <!--<![endif]-->
    ...
    <div class="navbar">
     ...
     <p >MENU GENERADOR DOCUMENTO DTE</p>
                     </div>
  ...
  <div class="content">
        ...
        <div class="movilButons">
         ...
         <button type="button" class="btn btn-success movile" <% if(perb_ingresar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='N<%=Constantes.MODULO%>_I_01_001'" <%} %> >INGRESAR</button>
         ...
         <button type="button" class="btn btn-warning movile" <% if(perb_modificar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='mgruper'" <%} %> >MODIFICAR</button>
         <button type="button" class="btn btn-primary movile" <% if(perb_consultar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='cguias_stipo'" <%} %>>CONSULTAR</button>
