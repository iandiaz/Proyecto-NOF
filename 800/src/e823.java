...
/**
 * Servlet implementation class e823
 */
@WebServlet("/e823")
public class e823 extends HttpServlet {
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		if(request.getParameter("grabar") != null){
			...
			try {
				...
				String updel="update `dte_encabezado` set `dte_encabezado`.`eliminado`=1 where `dte_encabezado`.`Codigo_relacionado`='"+DID+"' and `dte_encabezado`.`Modulo`='823'";
				state2.executeUpdate(updel);
				
				String del823det="update `detalle_823` set `detalle_823`.`estados_vig_novig_id`=2 where `detalle_823`.`823_id`='"+DID+"'";
				state2.executeUpdate(del823det);
		...
		RequestDispatcher rd = request.getRequestDispatcher("e823.jsp");
