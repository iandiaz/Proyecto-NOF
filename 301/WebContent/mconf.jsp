...
<div class="navbar">
	...
	<button type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='mconf_search'">VOLVER</button>
	<div align="center" class="title" >
		<p>MODIFICAR CONFIGURACION GPS DINAMICO</p>
	</div>
</div>       
<div class="content" >
...
<!-- <div class="divhead">
		 		<strong>ESTADO:</strong><select name="estados_vig_novig_id" id="estados_vig_novig_id" style="margin:0px 0px 0px 0px;width: 167px; margin-bottom: 9px">
	...
	<div class="divhead"><strong>Fec. última mod:</strong><input type="text" value="<%=fec_mod%>" style="width:225px;height:30px; background:#FFF;color: #000 " disabled="disabled">
					</div>
	<div class="divhead"><strong>Usu última mod:</strong><input type="text" value="<%=user_mod%>" style="width:400px;height:30px; background:#FFF;color: #000 " disabled="disabled">
					</div> -->
</div>
<br>
<table  class="table004det">
  ...
  <tr>
    ...
    <th scope="col" style="background:#39F; width: 630px;" >TIPO TICKET</th>
  </tr>
  </thead>
  <tbody class="scroll">
      <%
    String mod_actual="";  
         for(int i =0; i<ar_tticket.length; i++){
        	...
        	%>
  <tr>
    <td class="int" width="12px" ><%=usutic[0]%></td>
    <td class="int" width="12px" align="center"><input class="dale" name="usutti[]" <% if(usuario_ticket){%> checked="checked" <% } %> value="<%=usutic[0]%>" type="checkbox"></td>
    <td class="int" width="630px"><%=usutic[1]%></td>
  </tr>
        <%           	
         }
         %>
...
<table  class="table004det">
  ...
  <tr>
    ...
    <th scope="col" style="background:#39F; width: 12px;"><input type="checkbox" id="todos2" name="todos2"></th>
    <th scope="col" style="background:#39F; width: 600px;" >USUARIO GPS</th>
  </tr>
  </thead>
  <tbody class="scroll">
      <%
    //String mod_actual="";  
         for(int i =0; i<ar_usuarios.length; i++){
        	...
        	String[] usugps = ar_usuarios[i].toString().split("/");
        	boolean usuario_gps=false;
        	 for(int j =0; j<ar_usugps.length; j++){
        		 if(ar_usugps[j]!=null && !ar_usugps[j].equals("-1")){
            		 if(ar_usugps[j].equals(usugps[0])){  usuario_gps=true; break;}   
        		 }
        	 }
        	%>
  <tr>
    <td class="int" style="width: 46px;"><%=usugps[0]%></td>
    <td class="int" style="width: 12px;" align="center"><input class="dale2" name="usugps[]" <% if(usuario_gps){%> checked="checked" <% } %> value="<%=usugps[0]%>" type="checkbox"></td>
    <td class="int" style="width: 600px;"><%=usugps[1]%> <%=usugps[2]%></td>
  </tr>
        <%           	
         }
         %>
