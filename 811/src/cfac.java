	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		request.setAttribute("usuname", Usuarios_nombre);	
			...
			try {
				...
				String GID = request.getParameter("oc_id");
				//import java.io.IOException;
	    		...
	    		String SQL_CAB = "SELECT factura_compra_activo.oc_id, "
						...
						+ "		`factura_compra_activo`.`fac_comactivo_porcentaje_ivaret`, "
	 					+ "		`factura_compra_activo`.`fac_comactivo_valor_ivaret`,"
	 					+ "		`factura_compra_activo`.`fac_comactivo_valor_ivanoret`,	 "
			    		...
			    		+ " factura_compra_activo.fac_comactivo_neto  "					
						+ "	FROM"
			    		...
			    		+ "	inner join `estados_vig_novig` on `estados_vig_novig`.`estados_vig_novig_id`=factura_compra_activo.estados_vig_novig_id  "
			    		+ "	WHERE"
			    ...
			    if(CAB_RS.next()){
			    	...
			    	request.setAttribute("fac_comactivo_porcentaje_ivaret", CAB_RS.getString("fac_comactivo_porcentaje_ivaret"));
			    	request.setAttribute("fac_comactivo_valor_ivaret", CAB_RS.getString("fac_comactivo_valor_ivaret"));
			    	request.setAttribute("fac_comactivo_valor_ivanoret", CAB_RS.getString("fac_comactivo_valor_ivanoret"));
			    	request.setAttribute("FACT_ESTADO",CAB_RS.getString("estados_vig_novig_nombre"));
			    }
			    CAB_RS.close();
			    ...
			    Integer c=1;
			    while(DET_RS.next()){ prod.add(DET_RS.getString("productos_id")+"/"+DET_RS.getString("dfca_descripcion").replace("/", " ")+"/"+DET_RS.getString("dfca_valor")); c=c+1;}
			    DET_RS.close();	
