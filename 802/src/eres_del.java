...
/**
 * Servlet implementation class eres_del
 */
@WebServlet("/eres_del")
public class eres_del extends HttpServlet {
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		Statement statedel = null;
		Statement statedeld = null;
		ResultSet RS_CAB = null;
		...
		try {
    		...
    		statedel = (Statement) ((java.sql.Connection) conexion).createStatement();
    		statedeld = (Statement) ((java.sql.Connection) conexion).createStatement();
    		String GRID=request.getParameter("guiaresumen_id");
    		
    		if(request.getParameter("delete_id") !=  null){
    			statedel = (Statement) ((java.sql.Connection) conexion).createStatement();
 				String UPDATE = "UPDATE `802` SET estados_vig_novig_id=2,802_fecmod=now(),802_modificador="+id_iusuario+" WHERE 802_id="+request.getParameter("guiaresumen_id");
 				System.out.println(UPDATE);
 				statedel.executeUpdate(UPDATE);
 				statedel.close();
 				String UPDATEDET = "UPDATE detalle_802 SET estados_vig_novig_id=2 WHERE 802_id="+request.getParameter("guiaresumen_id");
 				System.out.println(UPDATE);
				  ...
				  //insertamos logs
		    		String log_sql=""
		    				...
		    				+ " ) "
		    				+ " VALUES "
		    				+ " (now(), 'USUARIO ESTABLECE NO VIGENTE REGISTRO "+request.getParameter("guiaresumen_id")+" EN TABLA 802', '"+id_iusuario+"') ";
		    		state.executeUpdate(log_sql);
				
 				
 				
 				statedeld.executeUpdate(UPDATEDET);
 				statedeld.close();
 				RequestDispatcher redir = request.getRequestDispatcher("eres_search?eExito=OK");
    		...
    		String SQL_CAB = "SELECT 	g.802_folio,"
					...
					+ "		IF(`g`.`fec_ref` is null,'',`g`.`fec_ref`) as fec_ref "
		    ...
		    RS_CAB =  state.executeQuery(SQL_CAB);
		    if(RS_CAB.next()){
    		...
    		String SQL_DETALLE = "SELECT 802_ID,d802_cod, d802_TOTAL, DATE_FORMAT(d802_FECHA, '%d-%m-%Y') as GD_FECHA,d802_folio "
    				+ " FROM detalle_802 "
    				+ " WHERE 802_id = "+GRID+" ORDER BY d802_id";
		    System.out.println(SQL_DETALLE);
		    RS_DETALLE =  state.executeQuery(SQL_DETALLE);
		    ...
		    for(int x=0; x < gr_detalle.size(); x++){
		    	//System.out.println(gr_detalle.get(x));
		...
		RequestDispatcher rd = request.getRequestDispatcher("eres_del.jsp");
