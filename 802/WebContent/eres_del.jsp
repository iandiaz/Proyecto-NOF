...
<div class="navbar">
	...
	<button type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='/802/eres_search'">VOLVER</button> 
	<div align="center" class="title">
		<p>ELIMINAR FACTURA AFECTA - EXENTA GUIA RESUMEN</p>
	</div>
</div>
<div class="content" >
  <form action="" name="form1" id="form1" method="post"> 
  	<input type="hidden" name="delete_id" value="<%=guiaresumen_id%>" >
	...
	<div class=" cuadroblanco" style="height:80px;padding: 5px 5px 5px 5px;">
		...
		<div class="divhead"><strong>EMISOR:</strong><input type="text" style="width:130px;height:30px ;background:#FFF;" value="<%=emisor_nof %>" disabled></div>
	...
	<div class=" cuadroblanco" style="height:275px;margin-top: 5px;">
		...
		<div class="divhead"><strong>RUT:</strong><input type="text" style="width:145px;height:30px ;background:#FFF;" value="<%=empresas_rut %>" disabled></div>
		<div class="divhead"><strong>COD CLIENTE:</strong><input type="text" style="width:70px;height:30px ;background:#FFF;" value="<%=empresas_id %>" disabled></div>
		<div class="divhead"><strong>RAZON SOCIAL:</strong><input type="text" style="width:600px;height:30px ;background:#FFF;" value="<%=empresas_razonsocial %>" disabled></div>
	...
	<div class=" cuadroblanco" style="height:250px;margin-top: 5px;">
		...
		<div style=" max-height:300px; width:1000px; ">
			...
			<tbody id="fbody" class="scroll" style="border: 1px solid black">
			 ...
			 %>
					<tr class="hov">
						<td width="300px" ><a href="<%if(Alertas[3].indexOf("S")!=-1){%>/822/cserv?gd_id=<%=Alertas[3].replace("S", "")%><%}else{%> /821/cfac?gv_id=<%=Alertas[3].replace("A", "")%> <%} %>" target="_blank"><%=Alertas[4]%></a><input type="hidden" name="detalle_prev[]" value="<%=Alertas[3]%>;GUIA DE DESPACHO <%=Alertas[2]%>;1;<%=valString%>;0;<%=valString%>"></td>
	...
	<input type="hidden" name="ref_prev" id="ref_prev" value="">
			<input type="hidden" value="<%=empresas_rut%>" name="rut_prev">
		...
		<input type="hidden" name="dsco_prev" id="dsco_prev" value="<%=desc %>">
			<input type="hidden" name="total_prev" id="total_prev" value="<%=total%>">
			<input type="hidden" name="iva_prev" id="iva_prev" value="<%=iva%>">
			<input type="hidden" name="neto_prev" id="neto_prev" value="<%=neto %>">
			<input type="hidden" name="dir_prev" id="dir_prev" value="<%=direccion_nombre %>">
			<input type="hidden" name="obs_prev" id="obs_prev" value="<%=gr_obs %>">
			...
			<input type="hidden"  name="ciudad_prev" id="ciudad_prev" value="<%=gr_ciudad%>">
