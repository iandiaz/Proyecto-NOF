	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
			...
			try {
			  ...
			  //insertamos logs
	    		String log_sql=""
	    				...
	    				+ " ) "
	    				+ " VALUES "
	    				+ " (now(), 'USUARIO ESTABLECE COMO NO VIGENTE REGISTRO "+ID_FAC+" DE LA TABLA 804', '"+id_iusuario+"') ";
		...
		try {
    		...
    		String GID=request.getParameter("gd_id");
    		 String SQL_CAB = "SELECT "
		    		...
		    		+ "		`804`.`804_condiciones` as fac_servim_condiciones, "
 					...
 					+ "		`804`.`804_total` as fac_servotros_totalfinal, "
 					+ "		`804`.`804_descuento` as fac_servotros_descuento, "
		    		+ "		`804`.`804_glosa_descuento` as fac_servotros_glosa_desc, "
		    		+ "		CONCAT_WS(' ',`usuarios`.`Usuarios_nombre`,`usuarios`.`Usuarios_ape_p`,`usuarios`.`Usuarios_ape_m`) AS Usuarios_nombre_com, "
		    		+ " 	`804`.`804_neto` as fac_servotros_neto, "
		    		+ "		`804`.`804_iva` as fac_servotros_iva "
		    ...
		    if(CAB_RS.next()){
		    	...
		    	request.setAttribute("fac_servim_glosa_desc", CAB_RS.getString("fac_servotros_glosa_desc"));
