...
margin: 0 auto;
    ...
    $( document ).ready(function() {
    	...
    	<%for(FuncMascaraConfVO func:mascara.getFuncionalidadesMascara()){
    		for(ItemFuncMascaraConfVO item :func.getItems()){ %>
    			getProductos('<%=func.getId()%>','<%=item.getId()%>');
    		<%}
    	}%>
	...
	function validateGrabar(){
	 ...
	 function getProductos(id_func,id_item){
	        ...
	        .done(function( data ) {
	        		var productos_arr = $.parseJSON(data);
	          		...
	          		$.each( productos_arr, function( key, producto ) {
	          		 ...
	          		 $('#prod_item_'+id_item).select2("val","");
  ...
  <form action="" name="form1" method="post" id="form1" > 
  	<div class=" cuadroblanco" style="height:auto;margin-top: 5px;overflow: auto; min-height: 50px">
		<div style="padding: 5px 5px 5px 5px;">
			<div class="divhead vertical-middle"><strong>NOMBRE CONFIGURACION:</strong> <input type="text" maxlength="40" class="amarillo" name="confe_nombre" id="confe_nombre" style="width:420px;height:30px ;"  ></div>
		</div>
	</div>
	<%for(FuncMascaraConfVO func:mascara.getFuncionalidadesMascara()){ %>
