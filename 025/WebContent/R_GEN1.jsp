...
<%
ArrayList<ConfEquipoVO> rep=(ArrayList<ConfEquipoVO>)request.getAttribute("rep");
...
<p><strong>INFORME DE CONTROL: CONTENIDO CONFIGURACION</strong></p>
...
<%if(conf_filter.getId()!=null){%><p><b>CONFIGURACION:</b> <%=conf_filter.getNombre() %></p> <%} %>
   ...
   <%
   		for(ConfEquipoVO conf : rep){%>
   			<ul>
   				<li><b><%=conf.getId() %> - <%=conf.getNombre() %></b> </li> 
   				<li type="none"><ul>
   						<%for(ConfEquipoDetalleVO det : conf.getDetalleConf()){%>
   							<li><%=det.getFuncionalidad().getNombre()%> 
   							- <%=det.getProducto().getId() %> 
   							- <%=det.getProducto().getPn() %> 
   							- <%=det.getProducto().getDesc_corto() %> 
   							- REND: <%=det.getProducto().getVida_util_imp() %></li>
   						<%} %></ul></li>
   			</ul>
