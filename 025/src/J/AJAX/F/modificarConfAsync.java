 ...
 */
@WebServlet("/modificarConfAsync")
public class modificarConfAsync extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public modificarConfAsync() {
        super();
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		conf.setEstadoVigNoVig(new EstadosVigNoVigVO(estados_vig_novig_id));
		//==========ASOCIAMOS LOS PRODUCTOS DE ITEMS SELECCIONADOS =========///
			...
			for(FuncMascaraConfVO func :mascara.getFuncionalidadesMascara()){
				...
				for(ItemFuncMascaraConfVO item:func.getItems()){
					...
					if(id_producto!=null && !id_producto.equals("")){
						...
						conf_detalle.setProducto(new ProductoVO(id_producto));
		...
		for(ConfEquipoDetalleVO detalle:conf.getDetalleConf()){
			...
			if(ConfEquipoDetallesDAO.getConfiguracionDetalle(detalle).size()==0){
				ConfEquipoDetallesDAO.insertConfEquipoDetalle(detalle, Controlador.getUsuIDSession(request));
			}
			else{
				ConfEquipoDetallesDAO.updateConfEquipoDetalle(detalle, Controlador.getUsuIDSession(request));
		...
		AlertEventTriggeredDAO.insert(Constantes.MODULOID, Constantes.PMODIFICAR, Controlador.getUsuIDSession(request), conf.getNombre(), conf.getId());
