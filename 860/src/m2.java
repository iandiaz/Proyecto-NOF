		...
		if(request.getParameter("grabar") !=  null){
			...
			try {
	    		...
	    		Connection conexion=(Connection) DriverManager.getConnection
	    		 ...
	    		 String precio_guia_desp = request.getParameter("precio_guia_desp");
	    		  ...
	    		  if(findpp_RS.getInt("n")>0){
	    			  //ya existe
	    			  //update
	    			  String up =" UPDATE precio_guia_traslado set"
	    			  		+ "  pgt_cuc='"+cuc+"'"
	    			  		+ "  ,pgt_precio_guia_traslado='"+precio_guia_desp+"'"
	    			  		+ "  ,pgt_porcentaje='"+porcentaje+"'"
	    			  		+ "  ,pgt_modificador='"+U_ID+"'"
	    			  		+ "  ,pgt_fecmod=now()"
	    			  ...
	    			  String up =" INSERT INTO precio_guia_traslado (pgt_prod_id,pgt_cuc,pgt_precio_guia_traslado,pgt_porcentaje,pgt_creador,pgt_feccreacion)"
	    			  		+ "	VALUES ('"+id_prod+"','"+cuc+"','"+precio_guia_desp+"','"+porcentaje+"','"+U_ID+"',now())";
	    			  System.out.println(up);
		    			  state.executeUpdate(up);
	    		  ...
	    		  response.sendRedirect("/860?Exito=OK");
	    		  return;
		...
		try {
    		...
    		Connection conexion=(Connection) DriverManager.getConnection
    		 ...
    		 String id_prod = request.getParameter("prod_id");
    		  ...
    		  if(findpp_RS.getInt("n")>0){
    			  //ya existe
    			  String find_ppp = "SELECT * FROM precio_guia_traslado WHERE pgt_prod_id="+id_prod;
    			  ...
    			  request.setAttribute("porcentaje", "40");
    			  request.setAttribute("prod_nom", prod_nom);
