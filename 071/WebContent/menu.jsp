...
<html lang="en">
  ...
  if(request.getParameter("Exito")!=null && !request.getParameter("Exito").equals("")){
  		...
  		if(request.getParameter("Exito").equals("NOOK")){
			out.println("<script>alert('EL PRODUCTO INGRESADO YA EXISTE')</script>");
			//out.println("<script>window.location = '/001/'</script>");
		} 
   ...
   if(request.getParameter("NOOK")!=null && !request.getParameter("NOOK").equals("")){
 		if(request.getParameter("NOOK").equals("2")){
			out.println("<script>alert('DEBE SELECCIONAR PEDIDOS CON LA MISMA DIRECCION')</script>");
			//out.println("<script>window.location = '/001/'</script>");
		} 
  ...
  <div class="content">
         ...
         <div class="menu" style="min-height: 45px;">
        <div class="movilButons">
                
         <button type="button" class="btn btn-success movile" <% if(perb_ingresar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='I01'" <%} %> >INGRESAR</button>
         <button type="button" class="btn btn-danger movile" <% if(perb_eliminar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='E01'" <%} %> >ELIMINAR</button>
         <button type="button" class="btn btn-warning movile" <% if(perb_modificar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='M01'" <%} %> >MODIFICAR</button>
         <button type="button" class="btn btn-primary movile" <% if(perb_consultar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='C01'" <%} %>>CONSULTAR</button>
         <button type="button" class="btn btn-info movile" <% if(perb_reportes.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='R01'" <%} %>  >REPORTES</button>
        </div>
