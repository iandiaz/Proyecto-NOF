...
<html lang="en">
  ...
  if(request.getParameter("Exito")!=null && !request.getParameter("Exito").equals("")){
	...
	%>
    <meta charset="utf-8">
    <title>New Office - Guia de Despacho Venta Servicio</title>
  ...
  <!--<![endif]-->
    <div class="navbar">           
     ...
     <p >MENU FACTURA AFECTA - EXENTA SERVICIOS IMPRESION</p>
                     </div>
    </div>
  <div class="content">
        ...
        <div class="movilButons">
         ...
         <button type="button" class="btn btn-success movile" <% if(perb_ingresar.equals("0")){%> disabled="disabled" <%}else{%> onClick="loading();location='ifac1'" <%} %> >INGRESAR</button>
         <button type="button" class="btn btn-danger movile" <% if(perb_eliminar.equals("0")){%> disabled="disabled" <%}else{%> onClick="loading();location='efac1'" <%} %> >ELIMINAR</button>
         <button type="button" class="btn btn-warning movile" <% if(perb_modificar.equals("0")){%> disabled="disabled" <%}else{%> onClick="loading();location='mfac1'" <%} %> >MODIFICAR</button>
         <button type="button" class="btn btn-primary movile" <% if(perb_consultar.equals("0")){%> disabled="disabled" <%}else{%> onClick="loading();location='cfac1'" <%} %>>CONSULTAR</button>
         <button type="button" class="btn btn-info movile" <% if(perb_reportes.equals("0")){%> disabled="disabled" <%}else{%> onClick="loading();location='rpt'" <%} %>  >REPORTES</button>
        </div>
