...
public class R_GEN1 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<GrupoVO> rpt = ReportesDAO.getEmpresasStruc(empresa_, Controlador.getUsuIDSession(request));
		...
		if(request.getParameter("grabar_pdf") !=  null){
			try{
		        ...
		        Paragraph titulo=new Paragraph("INFORME DE CONTROL: REPORTE CLIENTE-PROVEEDOR",fontBolt); 
		        ...
		        for(GrupoVO grupo:rpt){
		        	...
		        	for(EmpresaVO empresa :grupo.getEmpresas()){
			            ...
			            Chunk data_empresa=new Chunk(empresa.getId()+" - "+empresa.getNombre_nof()+" ES CLI:"+(empresa.getEscliente().equals("1")?"SI":"NO")
			            		...
			            		+" - "+empresa.getResponsable().getNombre()+" "+empresa.getResponsable().getApe_p()+" "+empresa.getResponsable().getApe_m()
		...
		if(request.getParameter("grabar_excel") !=  null){
			try{
				...
				(row1.createCell(0)).setCellValue("REPORTE CLIENTE-PROVEEDOR"); //A3
		       ...
		       // style.setBorderTop((short) 6); // double lines border
				...
				(row3.createCell(0)).setCellValue("EMPRESA NOMBRE FANTASIA"); //A3
				(row3.createCell(1)).setCellValue("EMPRESA NOMBRE NOF"); 
				(row3.createCell(2)).setCellValue("EMPRESA RAZON SOCIAL");
				(row3.createCell(3)).setCellValue("EMPRESA RUT"); 
				(row3.createCell(4)).setCellValue("EMPRESA WEB"); 
				(row3.createCell(5)).setCellValue("ID EMPRESA"); 
				(row3.createCell(6)).setCellValue("ESTADO CLIENTE-PROVEEDOR");
				(row3.createCell(7)).setCellValue("ESTADO");
				(row3.createCell(8)).setCellValue("GRUPO");
				(row3.createCell(9)).setCellValue("RESPONSABLE CUENTA");
				...
				int countRow=4;
				 for(GrupoVO grupo:rpt){
			         	...
			         	for(EmpresaVO empresa : grupo.getEmpresas()){
			        		...
			        		(rowData.createCell(0)).setCellValue(empresa.getNombre());
			        		(rowData.createCell(1)).setCellValue(empresa.getNombre_nof());
			        		(rowData.createCell(2)).setCellValue(empresa.getRazonsocial());
			        		(rowData.createCell(3)).setCellValue(empresa.getRut());
			        		(rowData.createCell(4)).setCellValue(empresa.getWeb());
			        		(rowData.createCell(5)).setCellValue(empresa.getId());
			        		(rowData.createCell(6)).setCellValue(empresa.getEstadoClPr().getNombre());
			        		(rowData.createCell(7)).setCellValue(empresa.getEstadoVigNoVig().getNombre());
			        		(rowData.createCell(8)).setCellValue(grupo.getNombre());
			        		(rowData.createCell(9)).setCellValue(empresa.getResponsable().getNombre()+" "+empresa.getResponsable().getApe_p()+" "+empresa.getResponsable().getApe_m());
