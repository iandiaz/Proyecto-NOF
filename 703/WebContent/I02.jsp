...
String gastos_fact=(String)request.getAttribute("gastos_fact");
if(gastos_fact==null) gastos_fact="0";
...
margin: 0 auto;
    ...
    $( document ).ready(function() {
		...
		$('#tcambio').on('change', function() {
			$('#grabar').hide();
		});
		$('#tcambio').on('blur', function() {
			if($('#tcambio').val()==""){limpiarValores();}
			else{
				var subtotal = Number($('#subtotal_').val())*Number($('#tcambio').val())+Number($('#gastos_fact').val());
...
<div class="content" >
  ...
  <input type="hidden" name="dir_direccion" id="dir_direccion" value="">
	...
	<div class=" cuadroblanco" style="height:275px;margin-top: 5px;">
		<h3 class="subtitle" align="center" style="margin:0px 0px 0px 0px"><p style="margin:0px 0px;font-size:20px;background-color: #39F">DATOS ENCABEZADO PARA FACTURA</p></h3>
	...
	<div class="divhead"><strong>VALOR TIPO CAMBIO:</strong><input type="text" name="tcambio" id="tcambio" class="amarillo" maxlength="9" style="width:120px;height:30px ;background:#FF3;text-align: right;" value="<%=tcambio %>" onkeydown="return validateNum(event)"><span class="cabecera" style="color:#F00">*</span></div>				
	</div>
	...
	<div class=" cuadroblanco" style="height:335px;margin-top: 5px;">
		...
		<tr style="background-color:#0101DF;color:#FFFFFF">
			...
			<td width="116px" style="font-size:20px"><strong>PRECIO</strong></td>
		</tr>
		</thead>
		<tbody id="fbody" class="scroll" style="border: 1px solid black">
		 ...
		 <%       
		         ...
		         for(int i =0; i<arActivos.length; i++){
		        	 ...
		        	 int precio = (int)((Double.parseDouble(CF)+Double.parseDouble(CV))*Double.parseDouble(tcambio));
		        	 if(selected){
					...
					<tr class="hov" >
						...
						<td><input type="text" class="inputDetail" id="p7" name="detallePreciosCF[]" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="<%=dfDecimal2digitos.format(Float.parseFloat(CF)) %>"  ></td>
						<td><input type="text" class="inputDetail" id="p7" name="detallePreciosCV[]" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="<%=dfDecimal2digitos.format(Float.parseFloat(CV)) %>"  ></td>
						<td><input type="text" class="inputDetail" id="p7" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="<%=precio %>"  ></td>
					</tr>
					<tr class="hov" style="border-bottom: 3px solid black; ">
						...
						<td><input type="text" class="inputDetail" name="detalleImps[]" <% if(cont6_activo.indexOf(Activos[0]+";;"+Activos[6])!=-1){ if(selected)totalImp+=Integer.parseInt(cont6_difs.get(cont6_activo.indexOf(Activos[0]+";;"+Activos[6]))); %> value="<%=cont6_difs.get(cont6_activo.indexOf(Activos[0]+";;"+Activos[6])) %>" <%}else{%> value="0" <% } %> style="height:30px ;" readonly="readonly"></td>
						<td >&nbsp;</td>
						<td >&nbsp;</td>
						<td >&nbsp;</td>
		         ...
		         for(AnexoContratoVO anexo : anexosContratos){
		        	 ...
		        	 if( (anexo.getCargoFijoUnico()!=null && Integer.parseInt(anexo.getCargoFijoUnico())>0) ){
		        		 //dibujamos linea 
		        		 int precio = (int)((Double.parseDouble(anexo.getCargoFijoUnico()))*Double.parseDouble(tcambio));
        				 ...
        				 <tr class="hov" style="border-bottom: 1px solid black; ">
							...
							<td><input type="text" class="inputDetail" id="p7" name="detallePreciosAnexoCV[]" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="0"  ></td>
							<td><input type="text" class="inputDetail" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="<%=precio %>"  ></td>
						</tr>
        				 <%
		        	 }
		        	 
	        		 for(EstructuraCobroVO estructura : anexo.getEstructurasCobro()){
	        			 if(estructura.getCostoFijo()!=null || estructura.getCostoVariable()!=null){
	        				 //dibujamos linea 
	        				 String CF="0";
	        				 String CV="0";
	        				 if(estructura.getCostoFijo()!=null)CF=estructura.getCostoFijo();
	        				 if(estructura.getCostoVariable()!=null)CV=estructura.getCostoVariable();
	        				 int precio = (int)((Double.parseDouble(CF)+Double.parseDouble(CV))*Double.parseDouble(tcambio));
	    		        	 
	        				  total+=(Double.parseDouble(estructura.getCostoFijo())+Double.parseDouble(estructura.getCostoVariable()) );
	        				 %>
	        				 <tr class="hov" style="border-bottom: 1px solid black; ">
								...
								<td><input type="text" class="inputDetail" name="detallePreciosEstrCF[]" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="<%=CF %>"  ></td>
								<td><input type="text" class="inputDetail" name="detallePreciosEstrCV[]" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="<%=CV %>"  ></td>
								<td><input type="text" class="inputDetail" maxlength="8" style="height:30px ;background:#FFF;" readonly="readonly" value="<%=precio %>"  ></td>
							</tr>
	        				 <%
	        			 }
		         ...
		         <tr class="hov" style="border-bottom: 1px solid black; ">
					<td class="checkUbi">&nbsp;</td>
     				<td class="detailID" >&nbsp;</td>
					<td class="detailActivo">GASTOS FACT.</td>
					<td colspan="3">&nbsp;</td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td><input type="text" name="gastos_fact" id="gastos_fact" class="inputDetail" maxlength="8" style="height:30px ;background:#FF3;" value="<%=gastos_fact %>" onblur="$('#tcambio').blur()" ></td>
				</tr>
	...
	<div style="padding: 5px 5px 5px 5px;">
		<div class="divhead"><strong>SUBTOTAL:</strong><input name="subtotal" id="subtotal" type="text" style="width:150px;height:30px ;background:#FFF;text-align: right;" value="0"  readonly="readonly"><input id="subtotal_" type="hidden" value="<%=total %>"  readonly="readonly"></div>
		...
		<div class="divhead"><strong id="netolabel">AFECTO:</strong><input type="text" style="width:130px;height:30px ;background:#FFF;text-align: right;" readonly="readonly" value="" id="neto" name="neto"></div>
	...
	<div style="padding: 5px 5px 5px 5px;">
		<div class="divhead"><strong>N DE IMPRESIONES:</strong><input type="text" name="n_impresiones" id="n_impresiones" onkeydown="return validateNum(event)" class="amarillo" maxlength="10" style="width:130px;height:30px ;background:#FFF;text-align: right;" value="<%=totalImp%>" readonly="readonly"></div>
