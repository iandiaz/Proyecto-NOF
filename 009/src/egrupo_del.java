 ...
 */
@WebServlet("/egrupo_del")
public class egrupo_del extends HttpServlet {
...
public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		try {
    		...
    		state2 = (Statement) ((java.sql.Connection) conexion).createStatement();
    		 ...
    		 if(request.getParameter("delete_id") !=  null){
 				...
 				String UPDATE = "UPDATE gruposusu SET estados_vig_novig_id=2 WHERE gruposusu_id="+request.getParameter("delete_id");
 				System.out.println("delete : "+UPDATE);
 				state3.executeUpdate(UPDATE);
 				state3.close();
 				RequestDispatcher redir = request.getRequestDispatcher("egruper?Exito=OK");
		  ...
		  state2.close();
            ...
            + "                 AND `gruposusu_has_perfilusu`.`estados_vig_novig_id` = 1  ORDER BY `perfilusu`.`perfilusu_nombre`" ;
            ...
            while(PERFILES_RS.next()){
                perfiles.add(PERFILES_RS.getString("perfilusu_id")+"/"+PERFILES_RS.getString("perfilusu_nombre"));
		...
		RequestDispatcher rd = request.getRequestDispatcher("egruper_del.jsp");
