...
<html lang="en">
  ...
  <!--<![endif]-->
   <script type="text/javascript">
  	function detelePerfil(id_g,nombre_g){
  		if(confirm("¿ESTA SEGURO QUE DESEA ELIMINAR EL GRUPO DE PERFIL'"+nombre_g+"' ?")){
  			document.getElementById('delete_id').value=id_g;
  			document.getElementById('deletegruper').submit();
  		}
  ...
  <form id="deletegruper" action="" method="post" style="margin: 0px 0px 0px 0px;">
    ...
    <div class="navbar">
                    ...
                    <button type="button" onclick="location='/009/'" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;">VOLVER</button>
                   
                    <div align="center" class="title">
                    <p >ELIMINAR GRUPO DE PERFILES USUARIO PARA ALERTAS</p>
                    </div>
         ...
         <input maxlength="50" type="text" id="searchInput" class="form-control span12" style="width:565px;height:30px" autofocus="autofocus">
                    </div>
</div>
  <div class="content">
         ...
         <p  style="margin:0px 0px;font-size:20px;background-color: #39F">ELEGIR GRUPO DE PERFIL USUARIO PARA ALERTAS A ELIMINAR </p> </h3>
...
<tbody id="fbody" class="scroll" style="border: 1px solid black">
 ...
 <%
         ...
         for(int i =0; i<arGrupos.length; i++){
			...
			<tr class="hov" title="<%=Grupos[4]%>" style="color:<%=Grupos[2].equals("1") ? "#000" :"#FFF"%>;font-size:20px;background-color:<%=Grupos[2].equals("1") ? "#00FF00" :"#F00"%>;font-size:20px;" onclick="location='egrupo_del?id_g=<%=Grupos[0]%>';">
				<td width="47px"><%=Grupos[0]%></td>
				<td width="570px"><%=Grupos[1]%></td>
