...
<html lang="en"><head>
	...
	<script src="N031_M_01_001.js"></script>
		...
		if(<%=oc.getDetalle_oc().size()%>>0){
			 ...
			 if(calc_valores()){
				...
				if(!validate){
					...
					$.post( "modificarOCAsync", $( "#form1" ).serialize())
					    .done(function( data ) {
					    	if(data.trim()=="OK"){
	...
	function calc_valores(){
		...
		if($('#oc_tcambio_ref').val()==""){
			...
			if(validate){
				...
				var totalLinea_tc=<%=det.getPrecio_unit() %>*$('#cant_<%=det.getProducto().getId()%>').val();
				var totalLinea_cl=$('#oc_tcambio_ref').val()*<%=det.getPrecio_unit()%>*$('#cant_<%=det.getProducto().getId()%>').val();
				
				$('#total_us_<%=det.getProducto().getId()%>').val(totalLinea_tc);
				$('#total_cl_<%=det.getProducto().getId()%>').val(Math.round(Number(totalLinea_cl)));
				
				$('#oc_neto').val(Number($('#oc_neto').val())+Number(Math.round(Number(totalLinea_cl))));
				$('#oc_iva').val(Math.round(Number($('#oc_neto').val())*0.19) );
				$('#oc_total').val(Math.round(Number($('#oc_neto').val())*1.19) );
  ...
  <form action="" name="form1" method="post" id="form1" > 
  	...
  	<h3 class="subtitle" align="center" style="margin:0px 0px 0px 0px"><span style="margin:0px 0px;font-size:20px;">DATOS OC </span> </h3>
		...
		<div style="padding: 5px 5px 5px 5px;">
			...
			<div class="divhead vertical-middle"><strong>FECHA EMISION:</strong><input type="text" class="amarillo" maxlength="20" style="width:120px;height:30px ;" id="oc_fec_emision" name="oc_fec_emision" value="${oc.fec_emision}" readonly="readonly"><span class="cabecera" style="color:#F00"> *</span></div>
			<div class="divhead vertical-middle"><strong>FECHA RECEPCION:</strong><input type="text" class="amarillo" maxlength="20" style="width:120px;height:30px ;" id="oc_fec_recepcion" name="oc_fec_recepcion" value="${oc.fec_recepcion}" readonly="readonly"><span class="cabecera" style="color:#F00"> *</span></div>
			<div class="divhead vertical-middle"><strong>ESTADO:</strong><select id="estados_vig_novig_id" name="estados_vig_novig_id" style="width: 130px;">
	...
	<div class=" cuadroblanco" style="height:auto;margin-top: 5px;overflow: auto; min-height: 50px">
  	<h3 class="subtitle" align="center" style="margin:0px 0px 0px 0px"><span style="margin:0px 0px;font-size:20px;">DATOS PRODUCTOS SOLICITADOS </span> </h3>
		...
		<div class="divhead vertical-middle"><button type="button" onclick="addProducto('N<%=Constantes.MODULO%>_M_02_001')" class="btn btn-success addprod" ><i class="icon-plus"></i></button> 
		...
		<%
			for(OcDetalleVO detalle: oc.getDetalle_oc()){
				if(detalle.getEstadoVigNoVig().getId().equals("2")) continue;%>
		<tr>
			...
			<td style="font-size:20px"><button type="button" onclick="removeProducto('<%=detalle.getProducto().getId() %>','N<%=Constantes.MODULO%>_M_02_001')" class="btn btn-danger" ><i class="icon-remove"></i></button> </td>
	...
	<div class=" cuadroblanco" style="height:auto;margin-top: 5px;overflow: auto; min-height: 50px">
  	<h3 class="subtitle" align="center" style="margin:0px 0px 0px 0px"><span style="margin:0px 0px;font-size:20px;">TOTALES</span> </h3>
		...
		<div style="padding: 5px 5px 5px 5px;">
			...
			<div class="divhead vertical-middle"><strong>NETO:</strong><input type="text" maxlength="20" style="width:220px;height:30px ;" id="oc_neto" name="oc_neto" readonly="readonly" value="${oc.neto}" ><span class="cabecera" style="color:#F00"> *</span></div>
			<div class="divhead vertical-middle"><strong>IVA:</strong><input type="text" maxlength="20" style="width:220px;height:30px ;" id="oc_iva" name="oc_iva" readonly="readonly" value="${oc.iva}" ><span class="cabecera" style="color:#F00"> *</span></div>
			<div class="divhead vertical-middle"><strong>TOTAL:</strong><input type="text" maxlength="20" style="width:220px;height:30px ;" id="oc_total" name="oc_total" readonly="readonly" value="${oc.total}" ><span class="cabecera" style="color:#F00"> *</span></div>
