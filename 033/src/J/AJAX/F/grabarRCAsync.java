...
public class grabarRCAsync extends HttpServlet {
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		if(rc==null){
			...
			rc.setFec_recepcion(inverseFec(rc.getFec_recepcion()));
			rc.setFec_guia_despacho(inverseFec(rc.getFec_guia_despacho()));
			rc.setFecha_factura(inverseFec(rc.getFecha_factura()));
			
			
			rc.setEstadoVigNoVig(new EstadosVigNoVigVO("1"));
			
			
			//insertamos rc
			String id=RecepcionesCompraDAO.insertRC(rc, Controlador.getUsuIDSession(request));
			/*
	...
	public void saveRCStatus(RecepcionCompraVO rc, HttpServletRequest request){
		...
		for(OcDetalleVO detalle_oc :arr_detOC){
			...
			for(String serie:series){
				System.out.println("SERIE:"+serie+" IDDETOC:"+detalle_oc.getId()+" OC:"+id_oc);
