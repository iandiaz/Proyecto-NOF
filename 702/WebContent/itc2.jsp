...
<div class="navbar">
	...
	<button type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='/702/itc1'">VOLVER</button> 
		 ...
		 <%       
		         ...
		         for(int i =0; i<arActivos.length; i++){
					...
					<tr class="hov" >
						...
						<td><input type="text" class="inputDetail" id="p6_<%=Activos[0] %>_<%=Activos[6] %>" name="p6" <% if(periodos_para_tc[5]!=null){if(cont5_activo.indexOf(Activos[0]+";;"+Activos[6])!=-1){%> value="<%=cont5_values.get(cont5_activo.indexOf(Activos[0]+";;"+Activos[6])) %>" <%}else{%> value="0" <% } %> <% } else{%> value="ND" <% } %> style="height:30px ;" disabled></td>
						<td><input type="text" class="inputDetail" id="p7_<%=Activos[0] %>_<%=Activos[6] %>" name="p7_<%=Activos[0] %>;<%=Activos[6] %>" maxlength="8" <% if(cont6_activo.indexOf(Activos[0]+";;"+Activos[6])!=-1){%> value="<%=cont6_values.get(cont6_activo.indexOf(Activos[0]+";;"+Activos[6])) %>" <%}else{%> value="0" <% } %>   <%if(habilitarEdit){%> style="height:30px ;background:#FF3;" onkeydown="return validateNum(event)" onblur="changeContador(this,'<%=Activos[0] %>_<%=Activos[6] %>')" tabindex="1" <%}else{%> style="height:30px ;background:#FFF;" disabled="disabled" <% }%>    ></td>
					</tr>
					<tr class="hov" style="border-bottom: 2px solid black; ">
						<td class="detailID" ></td>
						<td class="detailActivo" ><input type="text" id="cont_init_<%=Activos[0] %>_<%=Activos[6] %>" onblur="$('#p7_<%=Activos[0] %>_<%=Activos[6] %>').blur();" name="cont_init_<%=Activos[0] %>;<%=Activos[6] %>"  maxlength="8" value="<%=Activos[7] %>" style="width:180px ;height:30px ;background:#FF3;" onkeydown="return validateNum(event)" placeholder="Contador inicial"  >
						</td>
						<td></td>
						...
						<td><input type="text" class="inputDetail" id="difcont6_<%=Activos[0] %>_<%=Activos[6] %>" name="difcont6_<%=Activos[0] %>;<%=Activos[6] %>" value="<%=dif_cont6 %>" style="height:30px ;" readonly="readonly">
						<script>document.getElementById('p7t').innerHTML=Number(document.getElementById('p7t').innerHTML)+Number(<%=dif_cont6 %>);
								...
								$('#cont_init_<%=Activos[0] %>_<%=Activos[6] %>').blur();
								</script></td>
					</tr>
		        	<% } %>
		        	</tbody>
