...
/**
 * Servlet implementation class itc2
 */
@WebServlet("/itc2")
public class itc2 extends HttpServlet {
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		try {
    		...
    		if(request.getParameter("grabar") !=  null){
    			...
    			String SQL_UPDATE = "update `toma_contador` set `toma_contador`.`estados_vig_novig_id`=2 where `toma_contador`.`tc_periodo_id`="+request.getParameter("id_per");
    			System.out.println(SQL_UPDATE);
 			    state2.executeUpdate(SQL_UPDATE);
 			 ...
 			 //EXTRAEMOS FOTO ACTUAL DE ACTIVOS PARA EL PERIODO ACTUAL
			    ...
			    while(ACTIVO_RS.next()){
			    	String dif=request.getParameter("difcont6_"+ACTIVO_RS.getString("ALT_ID")+";"+ACTIVO_RS.getString("tp_tc_id"));
			    	String contInit=request.getParameter("cont_init_"+ACTIVO_RS.getString("ALT_ID")+";"+ACTIVO_RS.getString("tp_tc_id"));
			    	//System.out.println(dif+" - "+contInit);
			    	if(contInit!=null && !contInit.equals("")){
			    		System.out.println(contInit);
			    		//seteamos contador inicial, buscamos si existe, caso contrato lo insertamos 
			    		
			    		
			    		String SQL_FIND="select count(*) as n from contador_inicial where id_activo="+ACTIVO_RS.getString("ALT_ID")+" AND id_ubi='"+ACTIVO_RS.getString("UBI_ID")+"' AND tp_tc_id="+ACTIVO_RS.getString("tp_tc_id");
			    		ResultSet CI_RS = state3.executeQuery(SQL_FIND);
			    		CI_RS.next();
			    		if(CI_RS.getInt("n")==0){
			    			//no existe, insert
			    			String in="INSERT INTO contador_inicial (id_activo,id_ubi,ci_contador,tp_tc_id) "
			    					+ "	VALUES("+ACTIVO_RS.getString("ALT_ID")+","+ACTIVO_RS.getString("UBI_ID")+","+contInit+","+ACTIVO_RS.getString("tp_tc_id")+") ";
			    			state2.executeUpdate(in);
			    		}else{
			    			//existe, update
			    			
			    			String in="UPDATE contador_inicial SET ci_contador="+contInit+" WHERE  id_activo="+ACTIVO_RS.getString("ALT_ID")+" AND id_ubi="+ACTIVO_RS.getString("UBI_ID")+" AND tp_tc_id="+ACTIVO_RS.getString("tp_tc_id");
			    			state2.executeUpdate(in);
			    		}
					    
			    	}		
			    	//insertamos tabla toma contadores 
			    	String SQL_IN="INSERT INTO `toma_contador` "
			    			+ "		(`toma_contador`.`tc_activo_id`,`toma_contador`.`tc_periodo_id`,`toma_contador`.`tc_valor`,`toma_contador`.`tc_valor_diferencia`,`toma_contador`.`tc_fechain`,`toma_contador`.`tp_tc_id`)"
			    			+ "		VALUES ("+ACTIVO_RS.getString("ALT_ID")+","+request.getParameter("id_per")+",'"+request.getParameter("p7_"+ACTIVO_RS.getString("ALT_ID")+";"+ACTIVO_RS.getString("tp_tc_id"))+"','"+dif+"',now(),'"+ACTIVO_RS.getString("tp_tc_id")+"')";
			    	System.out.println(SQL_IN);
			    	state2.executeUpdate(SQL_IN, Statement.RETURN_GENERATED_KEYS);
			    	 
			    	 ResultSet generatedKeys = null;
					    generatedKeys = state2.getGeneratedKeys();
					    //String id_g="";
			    		...
			    		String log_sql=""
			    				...
			    				+ " ) "
			    				+ " VALUES "
			    				+ " (now(), 'USUARIO INGRESA REGISTRO "+GID+" DE LA TABLA toma_contador', '"+Controlador.getUsuIDSession(request)+"') ";
			    		state2.executeUpdate(log_sql);
			    ...
			    conexion.close();
 				...
 				RequestDispatcher redir = request.getRequestDispatcher("menucon?Exito=OK");
 				...
 				if(request.getParameter("id_per") !=  null){
 					//EXTRAEMOS FOTO ACTUAL DE ACTIVOS PARA EL PERIODO 
 				   ...
 				   String SQL_ACTIVO = ""
				    		...
				    		+ "		, `tipo_contador_principal`.`tp_tc_id`"
				    		+ "		,`tipo_contador_principal`.tp_tc_nombre"
				    		+ "		,IF(contador_inicial.ci_contador IS NULL ,0, contador_inicial.ci_contador) as ci_contador   "
				    		+ " FROM activos_historia a"
				    		+ " INNER JOIN producto p ON p.PROD_ID = a.PROD_ID "
				    		+ " INNER JOIN funcionalidad f ON f.FUNC_ID = p.FUNC_ID"
				    		+ " INNER JOIN ubicacion u ON u.UBI_ID = a.UBI_ID"
				    		...
				    		+ "					LIMIT 6 "
				    		+ " ) perFinal "
				    		+ " LEFT JOIN contador_inicial  ON (contador_inicial.id_activo = a.id_activo AND contador_inicial.id_ubi=a.UBI_ID AND contador_inicial.tp_tc_id=tipo_contador_principal.tp_tc_id)"
				    		+ " WHERE p.FUNC_ID = 6  "
				    		+ " AND a.empresas_id = "+request.getParameter("empresas_id")+""
 				    ...
 				    while(ACTIVO_RS.next()){
 				    	activos.add(ACTIVO_RS.getString("ALT_ID")
 				    			...
 				    			+";;"+ACTIVO_RS.getString("ci_contador")
 				    			
 				    			);
 				    	//TODO AGREGAR DIF Y CONT INICIAL 
 				 ...
 				 //--------------------- PERIODOS ----------------------//
				     String SQL_PERIODOS = "SELECT"
		 			    		...
		 			    		+ "		AND `periodos_tc`.`peri_tc_id_emp`="+request.getParameter("empresas_id")
		 			    		+ "		AND `periodos_tc`.`peri_tc_id_estperiodo`=1";
 				...
 				conexion.close();
			    state.close();
			    RequestDispatcher rd = request.getRequestDispatcher("itc2.jsp");
