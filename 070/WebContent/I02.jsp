...
String id_cliente=(String)request.getAttribute("id_cliente");
...
margin: 0 auto;
    ...
    $( document ).ready(function() {
			...
			getDireccionesSelect('<%=id_cliente%>');
    ...
    function getDireccionesSelect(id_cli){
   	 	...
   	 	$.post( "getDireccionesSelect", {id_cliente: id_cli <%if(direcciones_id!=null){%> ,direcciones_id: '<%=direcciones_id%>' <%} %>  })
	        .done(function( data ) {
	        	$( ".direccionesData" ).html( (data) );
	        	
	        	<%if(direcciones_id!=null){%> $('#direcciones_id').select2("val","<%=direcciones_id%>");<%}
	        	else{%> $('#direcciones_id').select2("val",""); <%} %>
	...
	function validateSubmitGrabar(){
    	...
    	if($('#prod_id').val()!=null && $('#prod_id').val()!=""){
    		...
    		if(<%=pedido.getDetallePedido().size()%>==0){
        		msg+="DEBE SELECCIONAR AL MENOS UN PRODUCTO PARA GRABAR\n";
        		validate=false;
        	}
...
<div class="navbar">
	...
	<button type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='/070/I01'">VOLVER</button> 
...
<div class="content" >
  ...
  <div class=" cuadroblanco" style="height:125px;padding: 5px 5px 5px 5px;">
		...
		<div class="divhead"><strong>TICKET SELECCIONADO:</strong><input type="text" style="width:145px;height:30px ;background:#FFF;" readonly="readonly" value="${pedido.getId_ticket()}" > </div>
		<div class="divhead"><strong>ESTADO PEDIDO:</strong><input type="text" style="width:145px;height:30px ;background:#FFF;" readonly="readonly" value="POR INGRESAR" > </div>
	...
	<div class=" cuadroblanco" style="height:160px;margin-top: 5px;">
		...
		<div class="divhead"><strong>UBICACI&Oacute;N LINEA:</strong><select name="ubi_id_linea" id="ubi_id_linea" class="ubicacionesData" style="margin:0px 0px 0px 0px;width: 400px" >
	...
	<h3 class="subtitle" align="center" style="margin:0px 0px 0px 0px"><p style="margin:0px 0px;font-size:20px;background-color: #39F">PRODUCTOS SELECCIONADOS</p></h3>		
		...
		<tr style="background-color:#0101DF;color:#FFFFFF">
			...
			<td class="detailID" style="font-size:20px"><strong>CANTIDAD </strong></td>
		...
		<tr class='hov' >
			...
			<td class="detailID" style="font-size:20px"><strong><%=detallepedido.getCantidad() %> </strong></td>
