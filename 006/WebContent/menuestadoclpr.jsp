...
<html lang="en">
  ...
  if(request.getParameter("Exito")!=null && !request.getParameter("Exito").equals("")){
	...
	%>
    <meta charset="utf-8">
    <title>Menu estado cliente proveedor - New Office</title>
  ...
  <!--<![endif]-->
    ...
    <div class="navbar">
                    ...
                    <form method="get" action="Menuestadoclpr" style="margin: 0px 0px 0px 0px;">       
     ...
     <p >MENU ESTADO CLIENTE-PROVEEDOR</p>
                     </div>
  ...
  <div class="content">
         ...
         <button type="button" class="btn btn-success movile" <% if(perb_ingresar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='Iestadoclpr'" <%} %> >INGRESAR</button>
         <button type="button" class="btn btn-danger movile" <% if(perb_eliminar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='Eestadoclpr'" <%} %> >ELIMINAR</button>
         <button type="button" class="btn btn-warning movile" <% if(perb_modificar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='Mestadoclpr'" <%} %> >MODIFICAR</button>
         <button type="button" class="btn btn-primary movile" <% if(perb_consultar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='Cestadoclpr'" <%} %>>CONSULTAR</button>
         <button type="button" class="btn btn-info movile" <% if(perb_reportes.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='rptestclpr'" <%} %>  >REPORTES</button>     
