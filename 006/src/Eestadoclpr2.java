...
/**
 * Servlet implementation class Eestadoclpr2
 */
@WebServlet("/Eestadoclpr2")
public class Eestadoclpr2 extends HttpServlet {
...
public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		try {
    		...
    		if(request.getParameter("delete_id") !=  null){
    			...
    			if(Constantes.SYNCDB==1){
	    			...
	    			String  estado_v_nv="NO VIGENTE";
	    			 ...
	    			 String insert_birt="UPDATE [ESTADO_CLPR] "
	    			 		...
	    			 		+ "	WHERE"
	    			 		+ "		[ESTADO_CLPR].[ESTCLPR_ID] = "+request.getParameter("delete_id");
				...
				String UPDATE = "UPDATE estado_clpr SET estados_vig_novig_id=2,Estado_Clpr_accion_alertada=0,Estado_Clpr_ult_idper_exec=13, Estado_Clpr_modificador="+Usuarios_ID+",Estado_Clpr_feccreacion=now() WHERE Estado_Clpr_id="+request.getParameter("delete_id");
				System.out.println("delete : "+UPDATE);
				statedel.executeUpdate(UPDATE);
				statedel.close();
				conexion.close();
				RequestDispatcher redir = request.getRequestDispatcher("Eestadoclpr?EExito=OK");
				redir.forward(request, response);
		        return;
			}
    		
    		
    		 String SQL_GRUPO = " SELECT *, "
 		    		+ "		CONCAT_WS(' ' , "
		    		...
		    		+ " WHERE clpr.Estado_Clpr_id="+Integer.parseInt(request.getParameter("id_clpr"));
		    System.out.println("select: "+SQL_GRUPO);
		    Grupo_RS =  state2.executeQuery(SQL_GRUPO);
		    if(Grupo_RS.next()){ 
		    	request.setAttribute("Estado_Clpr_id",Grupo_RS.getString("Estado_Clpr_id"));
		    	request.setAttribute("Estado_Clpr_nombre",Grupo_RS.getString("Estado_Clpr_nombre"));
		...
		RequestDispatcher rd = request.getRequestDispatcher("Eestadoclpr2.jsp");
