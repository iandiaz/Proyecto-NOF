...
ArrayList<MaquinaContadorVO> maquinasContadores=(ArrayList<MaquinaContadorVO>)request.getAttribute("maquinasContadores");
...
margin: 0 auto;
    ...
    function validateSubmit(){
     ...
     function validate_checksUbis(){
  	   
  	   <%
  	   for(MaquinaContadorVO ubi: maquinasContadores){
  		   %>
  		   if($('#<%=ubi.getUbi_id()%>_<%=ubi.getActivo_tptc_id()%>_ubi').is(":checked") ){
  			
  			   $('#<%=ubi.getUbi_id()%>_<%=ubi.getActivo_tptc_id()%>_setEstr').show();
  			   
  		   }else{
  			   $('#<%=ubi.getUbi_id()%>_<%=ubi.getActivo_tptc_id()%>_setEstr').hide();
  		   }
  	   <% }
...
<div class="navbar">
	...
	<button type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='/013/M01'">VOLVER</button> 
...
<div class="content" >
  <form action="M02" name="form1" method="post"> 
	...
	<div class=" cuadroblanco" style="height:230px;margin-top: 5px;">
		...
		<div class="divhead"><strong>ESTADO:</strong><select name="estadovignovig">
				<%for(EstadosVigNoVigVO estado: estadosvignovig){%>
					<option value="<%=estado.getId()%>" <%if(estado.getId().equals(anexoContrato.getEstadoVignoVig().getId())){%> selected="selected" <%} %>><%=estado.getNombre() %></option>
				<% } %>
		</select></div>
...
<table  class="table004det">
  ...
  <tbody class="scroll">
    ...
    String mod_actual="";
        ...
        for(int i =0; i<maquinasContadores.size(); i++){ 
        	 
        	MaquinaContadorVO maqCon = maquinasContadores.get(i);
  ...
  <tr>
    ...
    <td class="int" align="center"><input class="dale ubis_checks" name="maqContador[]" id="<%=maqCon.getUbi_id()%>_<%=maqCon.getActivo_tptc_id()%>_ubi"  value="<%=maqCon.getUbi_id()%>_<%=maqCon.getActivo_tptc_id()%>" type="checkbox" <%if(selected){%> checked="checked" <%} %> ></td>
    ...
    <td class="int" width="80px"><span id="<%=maqCon.getUbi_id()%>_<%=maqCon.getActivo_tptc_id()%>_setEstr"><a  data-fancybox-type="iframe" title="CONFIGURA LAS ESTRUCTURAS DE COBRO ASOCIADAS A LA UBICACI&OacuteN <%=maqCon.getUbi_id()%>." class="various tooltipUI "  href="ubiestrM?ubi_id=<%=maqCon.getUbi_id()%>&activo_tptc_id=<%=maqCon.getActivo_tptc_id()%>" ><i class="icon-cog"></i></a>&nbsp;<span id="<%=maqCon.getUbi_id()%>_nestructuras"><%=estructurasAsignadas %></span></span></td>
