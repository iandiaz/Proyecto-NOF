...
/**
 * Servlet implementation class inc3
 */
@WebServlet("/inc3")
public class inc3 extends HttpServlet {
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		if(request.getParameter("grabar") != null){
			try {
	    		...
	    		statee = (Statement) ((java.sql.Connection) conexion).createStatement();

				//----------------- CABECERA -----------------------//
	    		
				//INSERTAMOS TODOS LOS DATOS EN NOF LA CABECERA DE LA NC
	    		...
	    		String fec_nc=request.getParameter("fec_nc");
	    		if(fec_nc==null){fec_nc="NULL";}
				else{ 
					String fecrefar[]=fec_nc.split("-");
					fec_nc ="'"+fecrefar[2]+"-"+fecrefar[1]+"-"+fecrefar[0]+"'";
					}
	            ...
	            String obs0=request.getParameter("obs0");
	            String obs1=request.getParameter("obs1");
	            String obs2=request.getParameter("obs2");
	            
	            String rz=request.getParameter("841_rz");
	            String rut=request.getParameter("841_rut");
	            String giro=request.getParameter("empresas_girocliente");
	            String direccion=request.getParameter("841_direccion");
	            String comuna=request.getParameter("841_comuna"); 
	            String empresas_idemisor=request.getParameter("empresas_idemisor"); 
	            ...
	            String razon_nc=request.getParameter("razon_nc");
	            String CodigoRef="";
	            
	            String total_ref=request.getParameter("total_ref");
	            
	          
	            if(request.getParameter("tref").equals("1"))CodigoRef="1";
	            if(request.getParameter("tref").equals("2"))CodigoRef="2";
    			if(request.getParameter("tref").equals("3"))CodigoRef="3";
    		
	    		String SQL_IN = ""; 
	    		
	    		if(request.getParameter("tref").equals("2")){
	    			 ...
	    			 SQL_IN = ""
								...
								+ "	)"
		 			    		...
		 			    		+ ",'"+request.getParameter("tipo_dteref")+"'"
		 			    		+ ", "+fec_ref+""
		 			    		+ ",'"+CodigoRef+"'"
		 			    		+ ", '"+request.getParameter("mcod")+"'"
		 			    		+ ",'CORRECCION TEXTO EN DOCUMENTO DE REFERENCIA'"
		 			    		+ ",'"+total_ref+"' "
		 			    		+ ",'"+request.getParameter("clpr_id")+"'"
		 			    		+ ","+fec_nc+""
		 			    		+ ",'"+SUBTOTAL+"'"
		 			    		+ ",'"+TOTAL+"'"
		 			    		+ ",'"+desc+"'"
		 			    		...
		 			    		+ ",'"+ciudad+"'"
		 			    		+ ",'"+cont_email+"'"
		 			    		+ ",'"+request.getParameter("nc_dondedice").toUpperCase()+"'"
		 			    		+ ",'"+request.getParameter("nc_debedecir").toUpperCase()+"'"
		 			    		
		 			    		+ ",'"+id_iusuario+"'"
		 			    		+ ",now() "
		 			    		+ ",'"+razon_nc.toUpperCase()+"'"
		 			    		
		 			    		+ ")";
		    			  System.out.println(SQL_IN);
		  			    statee.executeUpdate(SQL_IN,Statement.RETURN_GENERATED_KEYS);
		    			
				    
				    ResultSet generatedKeys = null;
		    		  generatedKeys = statee.getGeneratedKeys();
		    		  String GID="";
	    			...
	    			//----------------- DETALLE -----------------------//
		    				  ...
		    				  String[] detnc = request.getParameterValues("detalle_nc[]");
		    		if(detnc!=null)
							    	
		  			    		for (String id_det: detnc) {
		  			    			String det_data[]=id_det.split(";;");
		  			    			if(det_data.length>0){
		  			    				String id_deta=det_data[0];
			  			    			String valor_real=det_data[1];
			  			    			String valor_nuevo=request.getParameter("detalle_nc_"+id_deta);
			  			    			String cod=request.getParameter("detalle_nc_cod_"+id_deta);
			  			    			String descripcion=request.getParameter("detalle_nc_desc_"+id_deta);
			  			    			
			  			    			System.out.println("ID:"+id_deta+" vreal:"+valor_real+" vnew:"+valor_nuevo);
			  			    					
			  			    			//capturamos el valor original, el valor nuevo y el id del detalle q corresponda
			  			    			String SQL_FACDET = "INSERT INTO detalle_841 (`detalle_841`.`841_id`,`detalle_841`.`d841_valor`,`detalle_841`.`d841_valorantiguo`,`detalle_841`.`d841_idref`,`detalle_841`.`d841_codigo`,`detalle_841`.`d841_descripcion`) "
			  			    					+ "							VALUES ("+GID+","+valor_nuevo+",'"+valor_real+"','"+id_deta+"','"+cod+"','"+descripcion+"')";
			  			    			System.out.println(SQL_FACDET);
			  							statee.executeUpdate(SQL_FACDET);	
		  			    			}
	    		...
	    		if(request.getParameter("tref").equals("1") || request.getParameter("tref").equals("3")){
	    			...
	    			if(request.getParameter("tref").equals("1"))textrefcod="ANULACION DOCUMENTO DE REFERENCIA";
	    			if(request.getParameter("tref").equals("3"))textrefcod="CORRECCION MONTO EN DOCUMENTO DE REFERENCIA";
	        	...
	        	response.sendRedirect("menunotacre?Exito=OK");
		...
		try {
			...
			String folioref="";
			String tipo_dteref="";
			String empresa_emisora="";
			
			if(request.getParameter("mcod").equals("801")){
				    	 ...
				    	 request.setAttribute("FACT_ID",GID);
				            ...
				            request.setAttribute("factura_obs2",RS_DATOS.getString("801_obs2"));
					 		...
					 		request.setAttribute("FACT_FOLIO", RS_DATOS.getString("801_FOLIO"));
					 		 ...
					 		 folioref=RS_DATOS.getString("801_FOLIO");
							 tipo_dteref=RS_DATOS.getString("801_tipodte");
							 empresa_emisora="123";
...
//				   
		             ...
		             rd = request.getRequestDispatcher("inc3_801.jsp");
		           
			 }
			 if(request.getParameter("mcod").equals("802")){
				 String GID=request.getParameter("fac_id");
				 ...
				 String SQL_CAB = "SELECT 	g.802_folio,"
				    ...
				    if(RS_CAB.next()){
				    	request.setAttribute("guiaresumen_id", GID);
				    	 ...
				    	 folioref=RS_CAB.getString("802_folio");
						 tipo_dteref=RS_CAB.getString("802_tipodte");
						 empresa_emisora=RS_CAB.getString("empresas_idemisor");
		    		...
		    		String SQL_DETALLE = "SELECT d802_id,802_ID,d802_cod, d802_TOTAL, DATE_FORMAT(d802_FECHA, '%d-%m-%Y') as GD_FECHA,d802_folio "
		    				+ " FROM detalle_802 "
		    				+ " WHERE 802_id = "+GID+" AND estados_vig_novig_id = 1 ORDER BY d802_id";
				    ...
				    while(RS_DETALLE.next()){
					    gr_detalle.add(RS_DETALLE.getString("802_ID")+"/"+RS_DETALLE.getString("d802_TOTAL")+"/"+RS_DETALLE.getString("GD_FECHA")+"/"+RS_DETALLE.getString("d802_cod")+"/"+RS_DETALLE.getString("d802_folio")+"/"+RS_DETALLE.getString("d802_id"));
		    	    }
				    ...
				    //-------------------------DETALLE GUIA RESUMEN-------------------------//
		             ...
		             rd = request.getRequestDispatcher("inc3_802.jsp");
		           
			 }
			 if(request.getParameter("mcod").equals("803")){
				 String GID=request.getParameter("fac_id");
				 String SQL_CAB = "SELECT "
				    ...
				    if(CAB_RS.next()){
				    	 ...
				    	 folioref=CAB_RS.getString("803_folio");
						 tipo_dteref=CAB_RS.getString("803_tipodte");
						 empresa_emisora=CAB_RS.getString("empresas_idemisor");
				    	...
				    	request.setAttribute("subtotal", CAB_RS.getString("803_subtotal"));
				    	request.setAttribute("neto", CAB_RS.getString("803_neto"));
				    	request.setAttribute("iva", CAB_RS.getString("803_iva"));
				    	request.setAttribute("total", CAB_RS.getString("803_total"));
				    ...
				    String SQL_DET = "SELECT * FROM detalle_803 WHERE 803_id = "+GID;
				    //System.out.println(SQL_DET);
				    ResultSet DET_RS = state.executeQuery(SQL_DET);
				    ArrayList<String> prod = new ArrayList<String>();
				    Integer c=0;
				    while(DET_RS.next()){ prod.add(c+"/"+DET_RS.getString("d803_descripcion")+"/"+DET_RS.getString("d803_valor")+"/"+DET_RS.getString("d803_id")); c=c+1;}
				    DET_RS.close();	
...
//				 
		             ...
		             rd = request.getRequestDispatcher("inc3_803.jsp");
		           
			 }
			 if(request.getParameter("mcod").equals("804")){
				 String GID=request.getParameter("fac_id");
				 String SQL_CAB = "SELECT "
				    ...
				    if(CAB_RS.next()){
				    	...
				    	folioref=CAB_RS.getString("fac_servim_folio");
						tipo_dteref=CAB_RS.getString("fac_servim_tipodte");
						empresa_emisora=CAB_RS.getString("empresas_idemisor");
				    	...
				    	request.setAttribute("subtotal", CAB_RS.getString("804_subtotal"));
				    	request.setAttribute("neto", CAB_RS.getString("804_neto"));
				    	request.setAttribute("iva", CAB_RS.getString("804_iva"));
				    	request.setAttribute("total", CAB_RS.getString("804_total"));
				    ...
				    ArrayList<String> prod = new ArrayList<String>();
				    Integer c=0;
				    while(DET_RS.next()){ prod.add(c+"/"+DET_RS.getString("d804_descripcion")+"/"+DET_RS.getString("d804_valor")+"/"+DET_RS.getString("d804_id")); c=c+1;}
					DET_RS.close();	
				    String[] ar_prod = new String[5];
				    for(int x=0; x < prod.size(); x++){ 
...
//				   
		             ...
		             rd = request.getRequestDispatcher("inc3_804.jsp");
		           
			 }
			 
			 
			//buscamos aca si existe otra nc para esta factura 
			 
				String SQLFIND="SELECT 841_id FROM `841` F "
						+ " WHERE "
						+ "		estados_vig_novig_id=1 "
						+ "		AND folioref="+folioref+" "
						+ "		AND tipo_dteref="+tipo_dteref+" "
						+ "		AND 841_empresa_emisora="+empresa_emisora+"  "
				 		+ "		AND CodigoRef<>2";
				...
				ResultSet RS_FINDNC= state.executeQuery(SQLFIND);
				
				if(RS_FINDNC.next()){
					request.setAttribute("ncexistente", RS_FINDNC.getString("841_id"));
				}
