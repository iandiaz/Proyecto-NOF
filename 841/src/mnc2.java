...
/**
 * Servlet implementation class mnc2
 */
@WebServlet("/mnc2")
public class mnc2 extends HttpServlet {
...
public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		if(request.getParameter("grabar") != null){
			...
			try {
	            ...
	            String nc_dondedice=request.getParameter("nc_dondedice").toUpperCase();
	            String nc_debedecir=request.getParameter("nc_debedecir").toUpperCase();
	    		...
	    		if(fec_nc==null){fec_nc="NULL";}
				 ...
				 String SQL_DATOS="UPDATE `841` "
				 		+ "	SET "
				 		+ "	estados_vig_novig_id="+request.getParameter("estados_vig_novig_id")
				 		+ "	,841_fecmod=now() "
				 		+ "	,841_modificador= "+id_iusuario
				 		+ "	,841_razon_nc = '"+razon_nc+"'"
				 		+ "	,841_fecha = "+fec_nc+"";
				 
						
				 if(request.getParameter("tref").equals("2")){
					 SQL_DATOS+= "	,841_dondedice='"+nc_dondedice+"'";
					 SQL_DATOS+= "	,841_debedecir='"+nc_debedecir+"'";
				 } 
				 if(request.getParameter("tref").equals("3")){
					 SQL_DATOS+="	,841_SUBTOTAL='"+SUBTOTAL+"'"
								+ "	,841_TOTAL='"+TOTAL+"'"
								+ "	,841_descuento='"+desc+"'"
								+ "	,841_NETO='"+NETO+"'"
								+ "	,841_IVA='"+IVA+"'";
				...
				SQL_DATOS+=" WHERE `841`.`841_id`="+request.getParameter("d_id");
				 
				System.out.println(SQL_DATOS);
				state.executeUpdate(SQL_DATOS);
				
				if(request.getParameter("tref").equals("3")){
					
					//removemos primero los detalles anteriores
					
					String SQL_FACDETDEL = "UPDATE `detalle_841` SET estados_vig_novig_id=2 "
		    					+ "							WHERE `detalle_841`.`841_id`="+request.getParameter("d_id");
		    		System.out.println(SQL_FACDETDEL);
					state.executeUpdate(SQL_FACDETDEL);		
			  			    		...
			  			    		for (String id_det: detnc) {
			  			    			...
			  			    			if(det_data.length>0){
				  			    			...
				  			    			String SQL_FACDET = "INSERT INTO detalle_841 (`detalle_841`.`841_id`,`detalle_841`.`d841_valor`,`detalle_841`.`d841_valorantiguo`,`detalle_841`.`d841_idref`,`detalle_841`.`d841_codigo`,`detalle_841`.`d841_descripcion`) "
				  			    					+ "							VALUES ("+request.getParameter("d_id")+","+valor_nuevo+",'"+valor_real+"','"+id_deta+"','"+cod+"','"+descripcion+"')";
				...
				response.sendRedirect("/841?Exito=OK");
			...
			request.setAttribute("d_id", request.getParameter("d_id"));
			 ...
			 String f_nc_sql="SELECT"
				    ...
				    if(f_nc_RS.next()){
				    	...
				    	request.setAttribute("total_ref", f_nc_RS.getString("total_ref"));
...
//						   
			           ...
			           RequestDispatcher rd = request.getRequestDispatcher("mnc2_801.jsp");
				    ...
				    if(modref.equals("802")){
						...
						//-------------------------CABECERA GUIA RESUMEN-------------------------//
						 String SQL_CAB = "SELECT 	g.802_folio,"
				            ...
				            RequestDispatcher rd = request.getRequestDispatcher("mnc2_802.jsp");
				            rd.forward(request, response);
					 }
...
//						   
				            ...
				            RequestDispatcher rd = request.getRequestDispatcher("mnc2_803.jsp");
				            rd.forward(request, response);
					 }
...
//						   
				            statecor.close();
				            conexion.close();
				            RequestDispatcher rd = request.getRequestDispatcher("mnc2_804.jsp");
				            rd.forward(request, response);
					 }
