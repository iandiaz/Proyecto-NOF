...
public class removeProductosAsync extends HttpServlet {
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		if(specialbid==null){
			...
			if(mod==null){
				specialbid.getProductos().remove(ProductosDAO.getProducto(id_prod));
			}
			else{
				if(mod.equals("I"))specialbid.getProductos().remove(ProductosDAO.getProducto(id_prod));
				if(mod.equals("M")){
					ProductoVO prodRemove = specialbid.getProductos().get(specialbid.getProductos().indexOf(new ProductoVO(id_prod)));
					prodRemove.setEstadoVigNoVig(new EstadosVigNoVigVO("2"));
				}
			}
	...
	public void saveSBStatus(SpecialBidVO sb_session, HttpServletRequest request){
		...
		sb_session.setTipo_moneda_id(Integer.parseInt(tipo_cambio_id));
