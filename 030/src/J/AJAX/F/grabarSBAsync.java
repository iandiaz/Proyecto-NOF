 ...
 */
@WebServlet("/grabarSBAsync")
public class grabarSBAsync extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public grabarSBAsync() {
        super();
	...
	public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		if(specialbid==null){
			...
			specialbid.setEstados_vig_novig(new EstadosVigNoVigVO("1"));
			//insertamos sb
			
			String id=SpecialBidsDAO.insertSpecialBid(specialbid, Controlador.getUsuIDSession(request));
			...
			if(id!=null){
				...
				AlertEventTriggeredDAO.insert(Constantes.MODULOID, Constantes.PINGRESAR, Controlador.getUsuIDSession(request), specialbid.getEmpresa(), specialbid.getId()+"");
				session.invalidate();
				out.println("OK");
			}
			else{
