...
margin: 0 auto;
    ...
    $( document ).ready(function() {
    	...
    	$('#estados_vig_novig_id').select2("val","<%=specialbid.getEstados_vig_novig().getId()%>");
		 ...
		 if(<%=specialbid.getProductos().size()%>>0){
			 ...
			 if(calc_valores()){
				...
				if(!validate){
					...
					$.post( "modificarSBAsync", $( "#form1" ).serialize())
					    .done(function( data ) {
					    	if(data.trim()=="OK"){
	...
	function addProducto(){
		...
		if(calc_valores()){
			if($('#prod_id option:selected').val()==''){
				 ...
				 $.post( "addProductosAsync?mod=M", $( "#form1" ).serialize())
			        .done(function( data ) {
			        	if(data.trim()=="OK") location="N<%=Constantes.MODULO%>_M_02_001";
			        	else if(data.trim()=="ERROR"){
	...
	function removeProducto(id_prod_remove){
		...
		if(calc_valores()){
				 ...
				 $.post( "removeProductosAsync?mod=M&id_prod_remove="+id_prod_remove, $( "#form1" ).serialize())
			        .done(function( data ) {
			        	if(data.trim()=="OK") location="N<%=Constantes.MODULO%>_M_02_001";
			        	else if(data.trim()=="ERROR"){
			        		alert("HA OCURRIDO UN ERROR POR FAVOR RECARGAR LA PAGINA");
  ...
  <form action="" name="form1" method="post" id="form1" > 
  	...
  	<h3 class="subtitle" align="center" style="margin:0px 0px 0px 0px"><span style="margin:0px 0px;font-size:20px;">DATOS SPECIAL BID </span> </h3>
		...
		<div style="padding: 5px 5px 5px 5px;">
			...
			<div class="divhead vertical-middle"><strong>ESTADO:</strong><select id="estados_vig_novig_id" name="estados_vig_novig_id" style="width: 150px" >
