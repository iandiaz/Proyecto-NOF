...
public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
			...
			try {
			    ...
			    state = (Statement) ((java.sql.Connection) conexion).createStatement();
				 ...
				 String SQL_DATOS="UPDATE `841` "
				 		+ "	Set "
				 		+ "	841_est_aprobacion=3 "
				 		+ "	,841_fechaaprobacion=now() "
				 		+ "	,841_usuaprobacion= "+id_iusuario
				...
				response.sendRedirect("/870?Exito=OK");
		...
		try {
    		...
    		String SQL_ESTADOS = "SELECT * FROM estado_aprobacion_nc";
    		System.out.println(SQL_ESTADOS);
		    ResultSet ESTADOS_RS = state.executeQuery(SQL_ESTADOS);
		    ArrayList<String> estados = new ArrayList<String>();
		    while(ESTADOS_RS.next()){ estados.add(ESTADOS_RS.getString("est_apro_nc_id")+"/"+ESTADOS_RS.getString("est_apro_nc_nombre")); }
		    ESTADOS_RS.close();
			...
			request.setAttribute("d_id", request.getParameter("d_id"));
			 ...
			 String f_nc_sql="SELECT"
						...
						+ "		estado_aprobacion_nc.est_apro_nc_nombre ,"
						
						+ "		DATE_FORMAT(`841`.`841_fechaaprobacion`,'%d-%m-%Y') as 841_fechaaprobacion ,"
						+ "		CONCAT_WS(' ',usuarios.Usuarios_nombre,usuarios.Usuarios_ape_p,usuarios.Usuarios_ape_m) as usuapro ,"
						+ "		`841`.`841_razon_nc`, "
						+ "		`841`.`841_n_impresiones` "
				 		...
				 		+ " LEFT JOIN usuarios ON usuarios.Usuarios_id=`841`.`841_usuaprobacion` "
				 		+ " LEFT JOIN estado_aprobacion_nc ON estado_aprobacion_nc.est_apro_nc_id=`841`.`841_est_aprobacion` "
				    ...
				    if(f_nc_RS.next()){
				    	...
				    	request.setAttribute("fechaaprobacion", f_nc_RS.getString("841_fechaaprobacion"));
				    	request.setAttribute("usuaprobacion", f_nc_RS.getString("usuapro"));
				    	...
				    	request.setAttribute("est_aprobacion", f_nc_RS.getString("est_apro_nc_nombre"));
...
//						   
			           ...
			           RequestDispatcher rd = request.getRequestDispatcher("e2_801.jsp");
				    ...
				    if(modref.equals("802")){
						...
						//-------------------------CABECERA GUIA RESUMEN-------------------------//
						 String SQL_CAB = "SELECT 	g.802_folio,"
				            ...
				            RequestDispatcher rd = request.getRequestDispatcher("e2_802.jsp");
				            rd.forward(request, response);
					 }
...
//						   
				            ...
				            RequestDispatcher rd = request.getRequestDispatcher("e2_803.jsp");
				            rd.forward(request, response);
					 }
...
//						   
				            statecor.close();
				            conexion.close();
				            RequestDispatcher rd = request.getRequestDispatcher("e2_804.jsp");
				            rd.forward(request, response);
					 }
