...
<html lang="en">
  ...
  if(request.getParameter("Exito")!=null && !request.getParameter("Exito").equals("")){
  		...
  		if(request.getParameter("Exito").equals("NOOK1")){
			out.println("<script>alert('EXISTEN FOLIOS UTILIZADOS DENTRO DEL RANGO SELECCIONADO, NO SE PUEDE ELIMINAR')</script>");
			//out.println("<script>window.location = '/001/'</script>");
		} 
  		if(request.getParameter("Exito").equals("NOOK2")){
			out.println("<script>alert('EXISTE UN RANGO POSTERIOR PARA ESTE TIPO DTE, NO SE PUEDE MODIFICAR')</script>");
			//out.println("<script>window.location = '/001/'</script>");
		} 
  ...
  <!--<![endif]-->
    ...
    <div class="navbar">
     ...
     <p >MENU ADMINISTRACION DE FOLIOS</p>
                     </div>
  ...
  <div class="content">
        ...
        <div class="movilButons">
         ...
         <button type="button" class="btn btn-success movile" <% if(perb_ingresar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='i1'" <%} %> >INGRESAR</button>
         <button type="button" class="btn btn-danger movile" <% if(perb_eliminar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='e1'" <%} %> >ELIMINAR</button>
         <button type="button" class="btn btn-warning movile" <% if(perb_modificar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='m1'" <%} %> >MODIFICAR</button>
         <button type="button" class="btn btn-primary movile" <% if(perb_consultar.equals("0")){%> disabled="disabled" <%}else{%> onClick="location='c1'" <%} %>>CONSULTAR</button>
