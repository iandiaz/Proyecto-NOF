        ...
        if (event.keyCode == 8 || event.keyCode == 46
         || event.keyCode == 37 || event.keyCode == 39 || event.keyCode == 9 || event.keyCode == 190 || (key >= 96 && key <= 105)) {
  		...
  		function ordenar(incialesUsu,tipo){
  			...
  			dataType: "text" ,
  			  type: 'GET', // o 'POST',
  			  data: { usu: incialesUsu, tipoOrden:tipo,estticket:<%=estticket%> , tipoticket:<%=tipoticket%>,f1:'<%=f1%>',f2:'<%=f2%>',emp:'<%=emp%>'},
  ...
  <body class=""> 
    ...
    <div class="navbar"> 
      ...
      <div class="brand"><img src="lib/bootstrap/img/logonew_big.png"></div>
                    ...
                    <button  type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='/302/m1'">VOLVER</button>
 ...
 <table  class="table004det">
  ...
  <tbody style="height: 80px;" class="toggle<%=usuarios_tick_ar[0].replace("/", "") %>" >
   ...
   <tr style="background-color: #00FFC4">
    ...
    <td class="int" width="195px"><b>EMPRESA</b></td>
    <td class="int" width="145px"><b>TIPO TICKET</b></td>
    <td class="int" width="110px"><b>D.NOF</b></td>
    <%if(estticket.equals("1")){%><td class="int" width="110px"><b>D.TEC</b></td><%} %>
	 ...
	 for(String tick : tickets){
		 ...
		 if(ar_t[0].equals(usuarios_tick_ar[0])){%>
			 <tr style="border-top: 1px solid #FFF" >
			   <td class="int" width="110px" ><%if(ar_t[16].equals("1")){%><a data-fancybox-type="iframe" href="ticketsdetectados?tick=<%=ar_t[1] %>" class="various"><img src="images/red.png" style="width: 20px" /></a><% }%>  <%=ar_t[1] %></td>
			   <td class="int" width="195px"><%=ar_t[3] %></td>
			    <td class="int" width="145px"><%=ar_t[8] %></td>
			    <td class="int" width="110px"><%=ar_t[5] %>Km</td>
			   <%if(estticket.equals("1")){%> <td class="int" width="110px"><%=ar_t[6] %>Km</td><%} %>
			    <td class="int" width="87px" style="background-color:<%=ar_t[11] %>; <%=ar_t[10] %>  "><%=ar_t[7] %></td>
			    <td class="int" width="43px"><input id="<%=ar_t[1] %>_r" name="<%=ar_t[1] %>_r" value="1" type="checkbox" <%if(ar_t[4].equals("1")){%> checked="checked" <%} %>  /> </td>
			    <%if(estticket.equals("1")){%><td class="int" ><input maxlength="3" type="text" class="amarillo" style="width:50px;height:30px ;background:#FF3;margin: 0px 0px 0px 0px;" onkeydown="return validateNum(event)" id="prio_<%=ar_t[1] %>" name="prio_<%=ar_t[1] %>" value="<%=ar_t[2].equals("null")?"":ar_t[2] %>" ></td><%} %>
			    <%if(estticket.equals("1")){%><td class="int" width="45px" ><input id="<%=ar_t[1] %>_visible" name="<%=ar_t[1] %>_visible" value="1" type="checkbox" <%if(ar_t[9].equals("1")){%> checked="checked" <%} %>  /></td><%} %>
