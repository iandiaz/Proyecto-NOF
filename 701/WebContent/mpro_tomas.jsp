...
<% 
  if(request.getParameter("Exito")!=null && !request.getParameter("Exito").equals("")){
  		...
  		if(request.getParameter("Exito").equals("NOOK1")){
			out.println("<script>alert('YA EXISTE UN PERIODO EN ESTADO ABIERTO')</script>");
			//out.println("<script>window.location = '/001/'</script>");
		} 
...
<div class="navbar">
	...
	<button type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='/701/mpro_search'">VOLVER</button> 
...
<tbody id="fbody" class="scroll" style="border: 1px solid black">
 ...
 <%        
         for(int i =0; i<arAlertas.length; i++){
        	 ...
        	 %>
        	<tr class="hov" style="color:<% if(Alertas[9].equals("1") || Alertas[9].equals("3")) out.print("#000");else if(Alertas[9].equals("2")) out.print("#FFF"); %>;font-size:20px;background-color:<% if(Alertas[9].equals("1")) out.print("#00FF00"); else if(Alertas[9].equals("2")) out.print("#F00");else if(Alertas[9].equals("3")) out.print("#FF0"); %>;font-size:20px;" onclick="location='mpro_mod?id_p=<%=Alertas[0]%>';" >			
