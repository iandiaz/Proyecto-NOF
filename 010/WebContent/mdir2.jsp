<%@ page language="java" contentType="text/html; charset=US-ASCII"
    ...
    String CUAD_ID=(String)request.getAttribute("CUAD_ID");
    String COMU_ID=(String)request.getAttribute("COMU_ID");
    String TIDIR_ID=(String)request.getAttribute("TIDIR_ID");
...
<div class="navbar">
	...
	<button type="button" class="btn btn-danger pull-right" style="margin-right: 5px; margin-top:5px;" onclick="location='/010/mdir1'">VOLVER</button> 
	<div align="center" class="title">
		<p >MODIFICAR DIRECCION</p>
	</div>
</div>
<div class="content" >
  <form action="" name="form1" method="post" onsubmit="return validateSubmit()" style="margin: 0px 0px 0px 0px">
  <div class=" cuadroblanco" style="height:200px;">
   ...
   <input type="text" value="<%=empresas_relacionada%>" style="width:45px;height:30px; background:#FFF; " disabled="disabled">   
	...
	<div class=" cuadroblanco" style="height:230px;margin-top: 2px;">
		...
		<div class="detailcb scroll">	
			...
			<%        
	       	 ...
	       	 %>
	       	 	<option value="<%=DireDato[0]%>" <% if(TIDIR_ID.equals(DireDato[0])){ %> selected="selected" <% } %>><%=DireDato[1]%></option>
	       	 <% } %>
			</select>  </div>
			<div class="divhead"><strong>COD POSTAL:</strong><input type="text" maxlength="7" name="cod_postal" id="cod_postal"  style="width:90px;height:30px ;"class="amarillo" value="<%=DIRE_COD_POSTAL%>" required="required" ><span class="cabecera" style="color:#F00">*</span></div>
			<div class="divhead"><strong>NOMBRE DIRECCION:</strong><input type="text" name="nom_dir" id="nom_dir" style="width:290px;height:30px ;" class="amarillo" maxlength="25" value="<%=DIRE_NOMBRE%>" required="required" ><span class="cabecera" style="color:#F00">*</span></div>		
			<div class="divhead"><strong>SECTOR:</strong><input type="text" name="sector" id="sector" style="width:250px;height:30px ;" class="amarillo" value="<%=DIRE_SECTOR%>" required="required" ><span class="cabecera" style="color:#F00">*</span></div>
			<div class="divhead"><strong>DIRECCION:</strong><input type="text" name="dir" id="dir" style="width:680px;height:30px ;" class="amarillo" maxlength="60" value="<%=DIRE_DIRECCION%>" required="required"><span class="cabecera" style="color:#F00">*</span></div>
			<div class="divhead"><strong>COMUNA:</strong><select id="comuna" name="comuna">
			<%        
				for(int i =0; i<ar_comus.length; i++){
	       	 	String[] DireDato = ar_comus[i].split("/");
	       	 %>
	       	 	<option value="<%=DireDato[0]%>" <% if(COMU_ID.equals(DireDato[0])){ %> selected="selected" <% } %>><%=DireDato[1]%></option>
	       	 <% } %>
			</select></div>
			<div class="divhead"><strong>REGION:</strong><input  type="text" style="width:520px;height:30px ;background:#FFF;" disabled="disabled" id="region" value="<%=REGI_NOMBRE%>"></div>
			<div class="divhead"><strong>CUADRANTE:</strong><select name="cua">
			<%        
				for(int i =0; i<ar_cuas.length; i++){
	       	 	String[] DireDato = ar_cuas[i].split("/");
	       	 %>
	       	 	<option value="<%=DireDato[0]%>" <% if(CUAD_ID.equals(DireDato[0])){ %> selected="selected" <% } %> ><%=DireDato[1]%></option>
	       	 <% } %>
			</select></div>
			<div class="divhead"><strong>CIUDAD:</strong><input type="text" name="ciudad" id="ciudad" style="width:187px;height:30px ;" class="amarillo" value="<%=DIRE_CIUDAD%>" required="required"><span class="cabecera" style="color:#F00">*</span></div>
			<div class="divhead"><strong>LAT:</strong><input type="text" maxlength="18" name="lat" id="lat" style="width:220px;height:30px ;" class="amarillo" value="<%=dire_lat%>" required="required"><span class="cabecera" style="color:#F00">*</span></div>
			<div class="divhead"><strong>LON:</strong><input type="text" maxlength="18" name="lon" id="lon" style="width:220px;height:30px ;" class="amarillo" value="<%=dire_lon%>" required="required"><span class="cabecera" style="color:#F00">*</span></div>
		
			<div class="divhead"><strong>SLA TEC:</strong><input type="text" onkeydown="return validateNum(event)" maxlength="7" name="nsa_tec" id="nsa_tec" style="width:100px;height:30px ;" class="amarillo" value="<%=dire_nsa_tecnico%>" required="required"><span class="cabecera" style="color:#F00">*</span></div>
			<div class="divhead"><strong>SLA LOG:</strong><input type="text" onkeydown="return validateNum(event)" maxlength="7" name="nsa_log" id="nsa_log" style="width:100px;height:30px ;" class="amarillo" value="<%=dire_nsa_logistica%>" required="required"><span class="cabecera" style="color:#F00">*</span></div>
			<div class="divhead"><strong>SLA PV:</strong><input type="text" onkeydown="return validateNum(event)" maxlength="7" name="nsa_pv" id="nsa_pv" style="width:100px;height:30px ;" class="amarillo" value="<%=dire_nsa_postventa%>" required="required"><span class="cabecera" style="color:#F00">*</span></div>
    ...
    <% } %>
			...
			<div class="divhead"><strong>USO DIRECCION:</strong>DESPACHO<input type="checkbox" name="use_dir_despacho" id="use_dir_despacho" <% if(use_dir_despacho.equals("1")){%> checked="checked" <%} %> value="1" > FACTURACION<input type="checkbox" name="use_dir_fac" id="use_dir_fac" <% if(use_dir_fac.equals("1")){%> checked="checked" <%} %> value="1" > COBRANZA<input type="checkbox" name="use_dir_cobranza" id="use_dir_cobranza" <% if(use_dir_cobranza.equals("1")){%> checked="checked" <%} %> value="1" > CORRESPONDENCIA<input type="checkbox" name="use_dir_corres" id="use_dir_corres" <% if(use_dir_corres.equals("1")){%> checked="checked" <%} %> value="1" > OTRO FIN<input type="checkbox" name="use_dir_otrofin" id="use_dir_otrofin" <% if(use_dir_otrofin.equals("1")){%> checked="checked" <%} %> value="1" ></div>
		</div>
	</div> 
	<div class="bgrabar">
