...
/**
 * Servlet implementation class mdir2
 */
@WebServlet("/mdir2")
public class mdir2 extends HttpServlet {
	...
	private void mt(HttpServletRequest request, HttpServletResponse response)throws IOException, ServletException {
		...
		PrintWriter out = response.getWriter();
				...
				if(request.getParameter("grabar") !=  null){
	    			...
	    			if(Constantes.SYNCDB==1){
	       			...
	       			String estado_v_nv="VIGENTE";
	       			 ...
	       			 String insert_birt="UPDATE [DIRECCION] "
	       					+ "	SET	[DIRECCION].[DIRE_ESTADO]='"+estado_v_nv+"',"
	       			 		
	       			 		+ "	 [DIRECCION].[COMU_ID] = '"+COMU_ID+"', "
	       			 		+ "	 [DIRECCION].[CUAD_ID] = '"+CUAD_ID+"', "
	       			 		+ "	 [DIRECCION].[DIRE_CIUDAD] = '"+DIRE_CIUDAD.toUpperCase().replace("'", " ")+"', "
	       			 		+ "	 [DIRECCION].[DIRE_COD_POSTAL] = '"+DIRE_COD_POSTAL.toUpperCase()+"', "
	       			 		+ "	 [DIRECCION].[DIRE_DIRECCION] = '"+DIRE_DIRECCION.toUpperCase().replace("'", " ")+"', "
	       			 		+ "	 [DIRECCION].[DIRE_FECHA_UM] = GETDATE(), "
	       			 		+ "	 [DIRECCION].[DIRE_NOMBRE] = '"+DIRE_NOMBRE.toUpperCase().replace("'", " ")+"', "
	       			 		+ "	 [DIRECCION].[DIRE_SECTOR] = '"+DIRE_SECTOR.toUpperCase().replace("'", " ")+"', "
	       			 		+ "	 [DIRECCION].[TIDIR_ID] = '"+TIDIR_ID+"', "
					...
					String sql_up="	"
							+ " UPDATE `direccion`"
							+ "	SET `direccion`.`COMU_ID` = '"+COMU_ID+"',"
							+ "		direccion.estados_vig_novig_id='"+estados_vig_novig_id+"', "
		   						
							+ "	 	`direccion`.`CUAD_ID` = '"+CUAD_ID+"',"
							+ "	 	`direccion`.`DIRE_CIUDAD` = '"+DIRE_CIUDAD.toUpperCase().replace("'", " ")+"',"
							+ "	 	`direccion`.`DIRE_COD_POSTAL` = '"+DIRE_COD_POSTAL+"',"
							+ "		`direccion`.`DIRE_DIRECCION` = '"+DIRE_DIRECCION.toUpperCase().replace("'", " ")+"',"
							...
							+ "		`direccion`.`DIRE_SECTOR` = '"+DIRE_SECTOR.toUpperCase().replace("'", " ")+"',"
							+ "		`direccion`.`TIDIR_ID` = '"+TIDIR_ID+"',"
							+ "		`direccion`.`dire_lat` = '"+lat+"', "
							+ "		`direccion`.`dire_lon` = '"+lon+"',"
							+ "		`direccion`.`dire_nsa_tecnico` = '"+nsa_tec+"',"
							+ "		`direccion`.`dire_nsa_logistica` = '"+nsa_log+"',"
							+ "		`direccion`.`dire_nsa_postventa` = '"+nsa_pv+"' , "
							+ "		`direccion`.`use_dir_despacho`='"+use_dir_despacho+"', "
							+ "		`direccion`.`use_dir_fac`='"+use_dir_fac+"' , "
							+ "		`direccion`.`use_dir_cobranza`='"+use_dir_cobranza+"' , "
							+ "		`direccion`.`use_dir_corres`='"+use_dir_corres+"' , "
							+ "		`direccion`.`use_dir_otrofin`='"+use_dir_otrofin+"'  "
							+ "	WHERE"
							+ "		`direccion`.`DIRE_ID` ="+dir_id;
				...
				try {
				    ...
				    while(clpr_rs.next()){     	   
				    	...
				    	request.setAttribute("CUAD_ID", clpr_rs.getString("CUAD_ID"));
				    	request.setAttribute("COMU_ID", clpr_rs.getString("COMU_ID"));	
				    	request.setAttribute("TIDIR_ID", clpr_rs.getString("TIDIR_ID"));	
				...
				RequestDispatcher rd = request.getRequestDispatcher("mdir2.jsp");
