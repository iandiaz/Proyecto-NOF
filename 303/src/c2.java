...
public void mt(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		try {
    		...
    		String find_usus_sql="SELECT Usuarios_inicial FROM usuarios WHERE Usuarios_id="+Controlador.getUsuIDSession(request);
    		ResultSet USUS_RS = state.executeQuery(find_usus_sql);
      		while(USUS_RS.next()){
      			iniciales+=",'"+USUS_RS.getString("Usuarios_inicial")+"' ";
    		...
    		String  ticketusu="SELECT" + 
    				...
    				if(estticket.equals("2")){
    		 		...
    		 		" )" + 
    		 		" AND [DETALLE_TECNICO_TICKET].[DIRE_ID]<>'' "+
